  ...
  chrome.contextMenus.removeAll(() => {
    chrome.contextMenus.create({
      ...
      if (chrome.runtime.lastError) {
        console.error('MENU_ERROR: Failed to create context menu:', chrome.runtime.lastError);
      } else {
...
// Main upload handler
async function handleImageUpload(imageUrl: string, pageUrl?: string, tab?: chrome.tabs.Tab) {
  ...
  try {
    ...
    if (!token) {
      throw new Error('Failed to obtain OAuth token. Please re-authorize the extension.');
    }
...
async function downloadImage(imageUrl: string): Promise<{ blob: Blob, filename: string }> {
  ...
  try {
    ...
    console.log('DOWNLOAD: Image downloaded, size:', blob.size, 'bytes, type:', blob.type);
    ...
    const validTypes = [
      ...
      'image/tiff', 'image/webp', 'image/heic', 'image/avif', 'image/x-icon'
    ];
...
function extractFilename(url: string): string | null {
  try {
    ...
    if (filename && /\.(jpg|jpeg|png|gif|bmp|tiff|webp|heic|avif|ico)$/i.test(filename)) {
      return filename;
    }
  } catch {
    // Invalid URL
...
async function createMediaItem(uploadToken: string, filename: string, token: string): Promise<any> {
